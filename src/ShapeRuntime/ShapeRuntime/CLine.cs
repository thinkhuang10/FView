...
public class CLine : CShape, ISupportHtml5
{
    ...
    public string makeCycleScript()
    {
        ...
        if (gdbh)
        {
            ...
            if (gdbhcankao == 1)
            {
                ...
                stringBuilder.AppendLine("line_Draw(\"" + ShapeName + "\"," + ShapeName + "_Point);");
                stringBuilder.AppendLine("}");
            }
            else
            {
    ...
    public string makeScript()
    {
        ...
        stringBuilder.AppendLine("$(\"#" + ShapeName + "\").data(\"get_line_Width\",function () {return get_line_Width(\"" + ShapeName + "\");});");
        stringBuilder.AppendLine("$(\"#" + ShapeName + "\").data(\"get_line_Height\",function () {return get_line_Height(\"" + ShapeName + "\");});");
        stringBuilder.AppendLine("$(\"#" + ShapeName + "\").data(\"get_line_Visible\",function () {return get_line_Visible(\"" + ShapeName + "\");});");
        stringBuilder.AppendLine("$(\"#" + ShapeName + "\").data(\"get_line_thick\",function () {return get_line_thick(\"" + ShapeName + "\");});");
        ...
        stringBuilder.AppendLine("$(\"#" + ShapeName + "\").data(\"set_line_thick\",function (value) { set_line_thick(\"" + ShapeName + "\");});");
        Cline_Pionts(stringBuilder);
        stringBuilder.AppendLine("line_Draw(\"" + ShapeName + "\"," + ShapeName + "_Point);");
    ...
    public void Cline_Pionts(StringBuilder sb)
    {
        List<GraphicsPath> list = new();
        if (swapgp != null && !needRefreshShape)
        {
        ...
        else
        {
            ...
            list.Add(graphicsPath);
            swapgp1 = graphicsPath;
        }
        if (swapgp1 != null)
        {
            sb.AppendLine("var  " + ShapeName + "_Point = new Array ()");
            for (int i = 0; i < swapgp1.PathData.Points.Length; i++)
            {
                sb.AppendLine(ShapeName + "_Point[" + i + "]= new  Array() ;");
                sb.AppendLine(ShapeName + "_Point[" + i + "][0]=" + swapgp1.PathData.Points[i].X);
                sb.AppendLine(ShapeName + "_Point[" + i + "][1]=" + swapgp1.PathData.Points[i].Y);
            }
        }
    }
}
